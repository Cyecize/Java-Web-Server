...
public class RedirectToLoginUrlHandler implements SecurityConfigHandler {
    @Override
    public void handle(HttpSoletRequest request, HttpSoletResponse response, SecurityConfig config) {
        response.sendRedirect(
                request.getContextPath() + config.getLoginURL() + "?callback=" + request.getRelativeRequestURL()
        );
    }
}
